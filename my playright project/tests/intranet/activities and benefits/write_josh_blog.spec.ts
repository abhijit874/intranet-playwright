...
test('write josh blog — happy flow', async ({ page }) => {
  const blogUrl = `https://joshsoftware.com/${Date.now()}`;
...
test('write josh blog — duplicate URL is rejected', async ({ page }) => {
  const blogUrl = `https://joshsoftware.com/${Date.now()}`;
  const activityDate = validCurrentQuarterDate();
  ...
  await cp.clickAddContribution();
  await cp.selectCategory('Blog Writing');
  await cp.fillTitle('playwright-josh-blog-original');
  await cp.fillDate(activityDate);
  await cp.fillFieldByLabel('Blog URL *', blogUrl);
  await page.locator('#published_on').selectOption('Josh blogpost');
  await cp.submitContribution();
  ...
  await cp.clickAddContribution();
  await cp.selectCategory('Blog Writing');
  await cp.fillTitle('playwright-josh-blog-dup');
  await cp.fillDate(activityDate);
  await cp.fillFieldByLabel('Blog URL *', blogUrl);
  await page.locator('#published_on').selectOption('Josh blogpost');
  await cp.submitContribution();
...
test('write josh blog — future date is rejected by the server', async ({ page }) => {
  const stamp = Date.now();
  const title = `playwright-josh-blog-future-${stamp}`; // unique per run, avoids collisions
...
test('write josh blog — previous-quarter date is rejected by the server', async ({ page }) => {
  const stamp = Date.now();
  const title = `playwright-josh-blog-prevq-${stamp}`; // unique per run, avoids collisions
...
// Self-contained: first creates a fresh Josh blog record, then opens that exact
...
test('edit existing josh blog contribution', async ({ page }) => {
  const originalTitle = `playwright-josh-blog-edit-${Date.now()}`;
