...
async function fillConferenceRequiredFields(page: import('@playwright/test').Page) {
  ...
  await page.getByLabel('Description *').fill('Lightning talk session');
...
test('submit conference lightning talk contribution', async ({ page }) => {
  ...
  await contributionsPage.selectSubcategory('Lightning Talk');
...
// Self-contained: creates a fresh Lightning Talk record with a unique title, then
// opens that exact record and edits it.
test('edit existing conference lightning talk contribution', async ({ page }) => {
  const originalTitle = `conference-lightning-${Date.now()}`;
...
test('conference lightning talk — future date is rejected by the server', async ({ page }) => {
...
test('conference lightning talk — previous-quarter date is rejected by the server', async ({ page }) => {
  const stamp = Date.now();
