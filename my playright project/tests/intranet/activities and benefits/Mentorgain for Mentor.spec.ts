...
test('mentorgain for mentor', async ({ page }) => {
  const title = `mentorgain-mentor-${Date.now()}`; // unique per run, avoids collisions
  ...
  await ldPage.selectLdSubcategory('35');
...
test('mentorgain for mentor — future date is rejected by the server', async ({ page }) => {
  const title = `mentorgain-mentor-future-${Date.now()}`; // unique per run, avoids collisions
...
test('mentorgain for mentor — previous-quarter date is rejected by the server', async ({ page }) => {
  const title = `mentorgain-mentor-prevq-${Date.now()}`; // unique per run, avoids collisions
