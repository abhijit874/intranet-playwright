...
test('edit existing public speaking meetup contribution', async ({ page }) => {
  ...
  const editable = await activitiesPage.openEditableFromSearchTerms(['Meetup', 'Public speaking', 'codex open AI'], { throwOnMissing: false });
  test.skip(!editable, 'Record is approved/locked. Skipping edit execution.');

  await activitiesPage.fillTitle(updatedTitle);
  await activitiesPage.fillFieldByLabel('Number of Attendees *', '75');
